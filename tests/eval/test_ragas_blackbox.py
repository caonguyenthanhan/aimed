"""RAGAS evaluation (blackbox HTTP)."""
...
def test_ragas_blackbox(llmops_settings, sample_dataset_path, llmops_eval_enabled: bool) -> None:
    if not llmops_eval_enabled:
        pytest.skip("LLMOps eval disabled. Enable via --llmops-eval or LLMOPS_EVAL_ENABLED=1.")

    base_env = str(llmops_settings.eval.blackbox.base_url_env or "").strip() or "CPU_SERVER_URL"
    if not (os.environ.get(base_env) or "").strip():
        pytest.skip(f"Missing env: {base_env}")
    ...
    sample_results = run_blackbox_samples(
        llmops_settings,
        samples=samples,
        user_id="eval-user",
        conversation_id_prefix=f"eval-blackbox-{uuid.uuid4().hex[:8]}",
        agent_id="auto",
        include_tools=True,
    )
    if not sample_results:
        pytest.skip("No blackbox results. Is the CPU server running and reachable?")

    out = run_ragas_evaluation(llmops_settings, sample_results=sample_results, run_id=f"blackbox-{uuid.uuid4().hex[:8]}")
