...
Tài liệu này trích xuất nguyên bản (raw text) các **System Prompts** và **Prompt Templates** đang được cấu hình và sử dụng trong hệ thống tư vấn y khoa và hỗ trợ tâm lý tích hợp đa tác tử (Multi-Agent System). 

Các chỉ thị này được chia làm các phần tương ứng với các tác tử cốt lõi trong hệ thống:
1. **Tác tử Điều phối & Sàng lọc (Supervisor / Triage Agent)**
2. **Tác tử Tâm lý (Psychological Agent - CBT & Bạn tâm giao)**
3. **Tác tử Y khoa (Medical Agent - RAG & Tra cứu)**
4. **Các Tác tử Hỗ trợ khác (Care Plan, Doctor Referral, Default)**

---

## I. TÁC TỬ ĐIỀU PHỐI & SÀNG LỌC (SUPERVISOR / TRIAGE AGENT)

Tác tử Điều phối và Sàng lọc chịu trách nhiệm tiếp nhận câu hỏi đầu tiên của người dùng, phân tầng nguy cơ lâm sàng (từ bình thường đến khẩn cấp), phát hiện các red-flags và điều phối sang các tác tử chuyên môn thích hợp.
...
* **Các biến số đầu vào**:
  * `{requested_agent_id}`: Định danh tác tử được yêu cầu thủ công (ví dụ: `"therapy"`, `"medication"`) hoặc mặc định là `"auto"` để tự động phân tích.
  * `{graph_summary_json}`: Dữ liệu tóm tắt các thực thể và quan hệ y khoa được truy xuất nhanh từ đồ thị tri thức (Graph Database).
  * `{user_text}`: Câu thoại đầu vào của người dùng.
...
* **Các biến số đầu vào**:
  * `TRIAGE_STATE_JSON`: Dữ liệu JSON chứa trạng thái phân tầng hiện tại (các triệu chứng đã thu thập, mức độ nguy cơ, các câu hỏi sàng lọc tiếp theo).
  * `TOOL_RESULTS_JSON`: Kết quả nén từ việc gọi các công cụ kiểm tra (như tra cứu kiến thức từ đồ thị hoặc tìm kiếm web).
  * `user_text`: Tin nhắn hiện tại của người dùng.
...
Các Agent trong LangGraph đều được bọc trong một khối hướng dẫn chuẩn (tiêu chuẩn xuất JSON và định dạng giao tiếp):
```text
Bạn là một AI agent chuyên nghiệp trong ứng dụng hỗ trợ y tế & tâm lý đa tác tử.
VAI TRÒ HIỆN TẠI CỦA BẠN: TRIAGE AGENT.
HƯỚNG DẪN VAI TRÒ:
[Nội dung hướng dẫn vai trò Triage ở trên]
...
Tác tử Tâm lý chịu trách nhiệm lắng nghe, thấu cảm, hỗ trợ tâm trạng, lo âu, stress và trầm cảm bằng phương pháp nhận thức hành vi (CBT) hoặc đóng vai trò bạn tâm giao để xoa dịu tinh thần.
...
* **Các biến số đầu vào**:
  * `TRIAGE_STATE_JSON`: Trạng thái sàng lọc của phiên.
  * `TOOL_RESULTS_JSON`: Chứa thông tin về Tác nhân kích thích (Triggers), Triệu chứng tâm lý (Symptoms), và gợi ý Chiến lược đối phó (Coping Strategies) được truy xuất từ Graph.
  * `user_text`: Tin nhắn của người dùng.
* **Nội dung hướng dẫn vai trò nguyên bản (Raw Role Instructions)**:
```text
...
* **Các biến số đầu vào**:
  * `{history}`: Lịch sử trò chuyện đời thường được nạp từ SQLite.
  * `{user_text}`: Lời nhắn hiện tại từ người dùng.
...
* **Các biến số đầu vào**:
  * `{question}`: Câu hỏi hoặc tình huống khó khăn của người dùng.
...
Tác tử Y khoa thực hiện nhiệm vụ cung cấp thông tin y khoa chính xác (bệnh lý, thuốc men, liều dùng, tương tác thuốc) dựa trên kết quả truy xuất dữ liệu đáng tin cậy.
...
* **Các biến số đầu vào**:
  * `TRIAGE_STATE_JSON`: Trạng thái sàng lọc của phiên.
  * `TOOL_RESULTS_JSON`: Chứa thông tin về thuốc và các tương tác thuốc tìm thấy trong GraphRAG hoặc Web Search.
  * `user_text`: Tin nhắn của người dùng.
* **Nội dung hướng dẫn vai trò nguyên bản (Raw Role Instructions)**:
```text
...
* **Các biến số đầu vào**:
  * `{user_query}`: Câu hỏi truy vấn y khoa của người dùng.
  * `{context_rag}`: Văn bản thô được kết hợp từ 5 nodes khớp nhất từ cơ sở dữ liệu ChromaDB.
  * `{mode_hint}`: Chỉ dẫn định hướng trọng tâm (ví dụ: Thuốc, Bệnh lý, Triệu chứng) phụ thuộc vào yêu cầu của người dùng.
* **Nội dung System Prompt nguyên bản (Raw System Prompt)**:
```text
...
[mode_hint]
```
Trong đó `{mode_hint}` được chèn động:
...
NGỮ CẢNH TRUY XUẤT (Context RAG):
{context_rag}

Dựa trên các hướng dẫn bạn nhận được (System Prompt) và Context RAG ở trên, hãy trả lời truy vấn của người dùng.
```
...
* **Các biến số đầu vào**:
  * `{question}`: Truy vấn y tế của người dùng.
...
### 4. Tác tử Tư vấn Y tế Gateway (API Fallback)
...
* **Các biến số đầu vào**:
  * `{p}`: Vai trò chi tiết (ví dụ: "Dược sĩ lâm sàng") truyền từ yêu cầu của người dùng.
  * `{determinedContext}`: Nhãn bối cảnh được tự động nhận diện từ tin nhắn (ví dụ: `psychological support`, `health lookup`).
...
### 5. Tác tử Y khoa Frontend (Medication Agent Profile - Cấu hình động)
...
Các tác tử bổ trợ nằm trong mô hình chăm sóc phân cấp (Stepped Care) để giúp người dùng duy trì thói quen hoặc lên kế hoạch điều trị.
...
* **Vị trí định nghĩa**:
  * Đồ thị LangGraph: [graph.py](file:///d:/desktop/tlcn/medical%20consulting%20system/cpu_server/langgraph_agent/graph.py#L815-L822)
  * Frontend Config: [prompt-config.json](file:///d:/desktop/tlcn/medical%20consulting%20system/medical-consultation-app/data/prompt-config.json#L33-L36)
...
* **Vị trí định nghĩa**:
  * Đồ thị LangGraph: [graph.py](file:///d:/desktop/tlcn/medical%20consulting%20system/cpu_server/langgraph_agent/graph.py#L823-L829)
  * Frontend Config: [prompt-config.json](file:///d:/desktop/tlcn/medical%20consulting%20system/medical-consultation-app/data/prompt-config.json#L37-L40)
* **Nội dung hướng dẫn vai trò nguyên bản (Raw Text)**:
```text
...
* **Vị trí định nghĩa**:
  * Đồ thị LangGraph: [graph.py](file:///d:/desktop/tlcn/medical%20consulting%20system/cpu_server/langgraph_agent/graph.py#L830-L833)
  * Frontend Config: [prompt-config.json](file:///d:/desktop/tlcn/medical%20consulting%20system/medical-consultation-app/data/prompt-config.json#L8-L12)
* **Nội dung hướng dẫn vai trò nguyên bản (Raw Text)**:
```text
